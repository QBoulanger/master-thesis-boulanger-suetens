...
class WumpusWorld(gym.Env):
    ...
    def step(self, action):
        ...
        if self.modify_reward:
            ...
            if self._location != self.gold and self.gold_reward_given:
                reward = -1000
        ...
        if self.modify_reward:
            ...
            if self._location != self.gold and self.gold_reward_given:
                ...
                done = self.env.is_done() or self.env.time_step == 100 or self.has_gold # TO FIND LUCAS SUETENS
            # done = self.env.is_done() or self.has_gold
        else:
            # The game is over with 4 conditions
            #   (1) Agent gets killed by wumpus
            #   (2) Agent falls into a pit
