"""
Grapher Program to observe mean score evolution based on num of environment calls
"""
...
env = "frozen-lake"  # "taxi", "frozen-lake" or "wumpus"
...
max_num_of_environment_calls = 1600000
max_num_of_environment_calls_b = 1500000
stats_every_x_steps = 100
num_of_runs = 5
validation_mean_over_x_episodes = 20

graph_title = "Comparison of Non-EVO Algorithms on Frozen Lake"
    ...
    for current_env_config in environment_configs:
        for j in range(num_of_runs):
            ...
            if algo_config["algo"] == "ngu":
                ...
                niceLegend = f"Baseline (ϵ = {algo_config['constant_x']})"
            ...
            while True:
                ...
                if len(already_existing_df) == 0:
                    ...
                    df_for_graph_generator.loc[len(df_for_graph_generator)] = [len(df_for_graph_generator),
                                                                               already_existing_df.iloc[0]["try"],
                                                                               legend,
                                                                               already_existing_df.iloc[0][
                                                                                   "num_of_environment_calls"],
                                                                               already_existing_df.iloc[0][
                                                                                   "average_reward"]]
                i += stats_every_x_steps
                num_of_environment_calls = already_existing_df.iloc[0]["num_of_environment_calls"]
    ...
    for df in all_trys:
        ...
        if merged_df.empty:
            ...
            for i, row in df.iterrows():
                ...
                merged_df.at[i, 'average_reward'] += row['average_reward']
                merged_df.at[i, 'max'] = max(merged_df.at[i, 'max'], row['average_reward'])
                merged_df.at[i, 'min'] = min(merged_df.at[i, 'min'], row['average_reward'])

    merged_df['num_of_environment_calls'] = merged_df['num_of_environment_calls'].div(len(all_trys))
    merged_df['average_reward'] = merged_df['average_reward'].div(len(all_trys))

    merged_df = merged_df.rename(columns={'average_reward': 'mean'})
...
for i, df in enumerate(list_of_current):
    ...
    sns.lineplot(x='num_of_environment_calls', y='mean', data=df_complete, label=f'{legendsMatching[legends[i]]}', color=color_dict[legends[i]])
    # plt.xlim(0, 100000)
    # plt.fill_between(df_complete['num_of_environment_calls'], df_complete['min'], df_complete['max'], alpha=0.2, color=color_dict[legends[i]])

plt.title(graph_title)
plt.xlabel('Number of Environment calls')
plt.ylabel('Average Cumulative Reward')
plt.xlim(0,max_num_of_environment_calls_b)
