"""
Grapher Program to observe epsilon evolution based on num of episodes
"""
...
graph_title = "Epsilon Evolution on wumpus-5"
        ...
        for j in range(num_of_runs):
            ...
            for i in range(0, max_num_of_episodes, stats_every_x_steps):
                ...
                if len(already_existing_df) == 0:
                    ...
                    df_for_graph_generator.loc[len(df_for_graph_generator)] = [len(df_for_graph_generator), legend, already_existing_df.iloc[0]["number_of_episodes"], already_existing_df.iloc[0]["mu_t_expl"] ]
...
for i in df_for_graph_generator['legend'].unique():
    ...
    grouped_df = current_df.groupby('number_of_episodes')['mu_epsilon'].agg(['mean', 'min', 'max']).reset_index()
