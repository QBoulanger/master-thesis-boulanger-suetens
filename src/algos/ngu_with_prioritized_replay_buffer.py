...
# NGU
class NGUWithPrioritizedReplayBuffer:
    ...
    def run(self, learning_rate=0.1, discount_factor=0.95, max_num_of_episodes=None, max_num_of_environment_calls=None,
        ...
        while True:
            ...
            for n in range(self.N):
                ...
                env = envs[n]
                env_state = env.reset()
                state = self.env.get_comparable_representation_of_state(env_state)
                done = False
                truncated = False
            ...
            for i in range(len(experiences)):  # for each experience
                ...
                self.replay_buffer.update_priorities([index], [get_loss(GENERAL_EXTRINSIC_Q_TABLE, state, next_state, action,
                                                         extrinsic_reward, discount_factor)])
...
if __name__ == '__main__':
    ...
    algo = NGUWithPrioritizedReplayBuffer(environment_generator)

    results = algo.run(0.9, 0.99, 1000, min_score=0.5, retrieve_stats=True, validation_mean_over_x_episodes=3)
