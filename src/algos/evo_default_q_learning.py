...
class EvoDefaultQLearning:
    ...
    def run(self, learning_rate=0.9, discount_factor=0.95, max_num_of_episodes=None, max_num_of_environment_calls=None,
        ...
        envs = [self.environment_generator() for _ in range(evo_N)]
        ...
        n_expl_par = sample_n(mu_t_expl, sigma_expl, evo_N)

        Qs = [{} for _ in range(evo_N)]
        Q_main = {}
        ...
        while True:
            ...
            for evo_n in range(evo_N):
                ...
                while not done and not truncated and num_of_steps < max_num_of_steps_per_episode:
                    comparable_state = env.get_comparable_representation_of_state(state)
                    self.add_q(Qs[evo_n], comparable_state)
                    self.add_q(Q_main, comparable_state)

                    self.state_visited_during_training(comparable_state)

                    action = self.epsilon_greedy(comparable_state, mu_t_expl, Qs[evo_n])
                    next_state, reward, done, truncated = env.make_step(action)
                    num_of_environment_calls += 1

                    comparable_next_state = env.get_comparable_representation_of_state(next_state)
                    self.add_q(Qs[evo_n], comparable_next_state)
                    self.add_q(Q_main, comparable_next_state)

                    transition = (comparable_state, action, float(reward), comparable_next_state)
                    Qs[evo_n] = self.f(Qs[evo_n], [transition], learning_rate, discount_factor)
                    Q_main = self.f(Q_main, [transition], learning_rate, discount_factor)

                    total_reward_list[evo_n] += get_loss(Qs[evo_n], comparable_state, comparable_next_state, action,
                                                         float(reward), discount_factor)
            ...
            if episode % 10 == 0:
                ...
                for evo_n in range(evo_N):
                    cumulative_expl += meta_obj[evo_n] * (n_expl_par[evo_n] - mu_t_expl)

                mu_t_expl = mu_t_expl + beta * (1 / (sigma_expl * evo_N)) * cumulative_expl
                mu_t_expl = np.clip(mu_t_expl, 0, 1)

                Qs = [copy.deepcopy(Q_main) for _ in range(evo_N)]
                total_reward_list = [0 for _ in range(evo_N)]
                n_expl_par = sample_n(mu_t_expl, sigma_expl, evo_N)

            # VALIDATION PART
            if (retrieve_stats and episode % retrieve_stats_every_x_steps == 0) or (
                ...
                for _ in range(validation_mean_over_x_episodes):
                    ...
                    while not done and not truncated:
                        ...
                        action = np.argmax(Q_main[comparable_state])
...
if __name__ == '__main__':
    ...
    algo = EvoDefaultQLearning(environment_generator)
    results = algo.run(0.1, 0.99, 1000, min_score=0.5, retrieve_stats=True, validation_mean_over_x_episodes=1,
                       retrieve_stats_every_x_steps=100)
